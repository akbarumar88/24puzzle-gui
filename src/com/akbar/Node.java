...
public class Node {
    ...
    /**
     * Initialise a search node. Cost is calculated from the count of the misplaced tiles.
     * @param state the state it contains
     * @param parent the parent node it derives from
     * @param action the action that we took to get here
     */
    ...
    public Node[] expand() {
        ...
        for (int a=0; a<successors.length; a++) {
            ...
            descendants[a] = new Node(child, this, action);
        }
        return descendants;
    }
    ...
    /**
     * Determine all actions (opposite) that were used to get to this node. Untuk mengenerate langkah previous.
     * @return all actions that took us to this node
     */
    public Action[] getActionsOpposite() {
        Action[] steps=new Action[depth];
        Node node=this;
        for (int i=0; i<depth; i++) {
            ...
            if (a.toString() == "UP") {
                a = new Action("DOWN");
            } else if (a.toString() == "DOWN") {
                a = new Action("UP");
            } else if (a.toString() == "RIGHT") {
                a = new Action("LEFT");
            } else if (a.toString() == "LEFT") {
                a = new Action("RIGHT");
            }
            steps[i]=node.getAction();
    ...
    /**
     *
     * @return score dari node ini ( f(n) )
     */
    public double getScore() {
        return this.cost + this.depth;
    }
    ...
    /**
     * Perform an A* Best First Search Algorithm.
     * @param initial Initial State
     * @return
     */
    public static Node AStar(State initial) {
        // add the initial state to the fringe
        ...
        while (!fringe.isEmpty()) { // test if fringe is empty, if yes "failure"
            // poll the first node in the list
            fringe = Helper.sortByFn(fringe); //Diurutkan berdasarkan f(n) terkecil.
            Node head=(Node)fringe.get(0);

            fringe.remove(0);
            closed.add(head); // Tambahkan current node ke daftar node yang sudah dikunjungi.
            // pull out the state in the node
            State state=head.getState();
            // examine it to see if it is a goal state
            if (state.goal()) {
            ...
            // expand the node, and add all new states to the fringe
            //fringe.clear(); //Bersihkan queue, untuk memastikan agar node selanjutnya yang akan dikunjungi adalah keturunan dari current node.
            Node[] successors = head.expand();
